...
export default async function ComparePage({ searchParams }: { searchParams: { ids?: string } }) {
  ...
  const rows: { label: string; render: (a: Application) => React.ReactNode }[] = [
    { label: 'Şirket', render: (a) => <span className="font-medium text-slate-700">{a.company_name}</span> },
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-slate-800">Başvuru Karşılaştırma</h1>
        <p className="text-sm text-slate-500">{ordered.length} başvuruyu yan yana görüyorsun</p>
      </div>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-slate-100">
              <th className="py-2 pr-4 text-left text-xs font-medium text-slate-400 w-32">Kriter</th>
              {ordered.map((a) => (
                <th key={a.id} className="px-3 py-2 text-left">
                  <Link href={`/applications/${a.id}`} className="hover:underline">
                    <p className="font-semibold text-slate-800 leading-tight">{a.position_title}</p>
