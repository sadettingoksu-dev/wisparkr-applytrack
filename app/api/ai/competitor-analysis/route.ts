...
import { getAnthropicClient, analyzeCompetition } from '@/lib/anthropic'
...
// Rakip analizi — Pro planına özel, aylık AI havuzundan metered.
export async function POST(request: Request) {
  ...
  if (!getPlan(profile.plan).features.competitorAnalysis) {
    return NextResponse.json(
      { error: { code: 'FEATURE_NOT_AVAILABLE', message: 'Rakip analizi yalnızca Pro planında mevcut.' } },
      { status: 403 }
    )
  }
  ...
  let result
  try {
    result = await analyzeCompetition(anthropic, {
    ...
    return NextResponse.json(
      { error: { code: 'AI_REQUEST_FAILED', message: 'Rakip analizi yapılamadı.' } },
      { status: 502 }
    )
  }
  ...
  await supabase
    .from('applications')
    .update({ competitor_analysis: result } as never)
    .eq('id', application_id)
    .eq('user_id', userId)
