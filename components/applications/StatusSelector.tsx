...
export function StatusSelector({ applicationId, initialStatus }: { applicationId: string; initialStatus: ApplicationStatus }) {
  ...
  return (
    <div className="relative">
      ...
      {open && (
        <div className="absolute right-0 top-full z-20 mt-1 w-40 rounded-lg border border-slate-100 bg-white shadow-lg">
          {KANBAN_COLUMNS.map((col) => (
            <button
              key={col.id}
              onClick={() => handleSelect(col.id)}
              className={`flex w-full items-center gap-2 px-3 py-2 text-sm hover:bg-slate-50 ${col.id === status ? 'font-semibold' : ''}`}
            >
