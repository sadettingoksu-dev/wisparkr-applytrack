 ...
 * kullanıcı linke tıklayıp doğrudan o sayfaya gider. Arayüz ve asistan yanıtı
 * kullanıcının seçili diline göre gelir (i18n + locale API'ye iletilir).
 */
...
export function ParkrcanWidget() {
  ...
  const [messages, setMessages] = useState<Message[]>(() => [{ role: 'assistant', text: p.greeting }])
  ...
  async function send() {
    ...
    try {
      ...
      if (!res.ok) {
        setMessages((m) => [...m, { role: 'assistant', text: json.error?.message ?? p.error }])
      } else {
        setMessages((m) => [
          ...m,
      ...
      setMessages((m) => [...m, { role: 'assistant', text: p.connError }])
    } finally {
      setLoading(false)
      scrollToBottom()
    }
        ...
        <div className="fixed bottom-24 right-5 z-40 flex h-[28rem] max-h-[calc(100vh-8rem)] w-[calc(100vw-2.5rem)] max-w-sm flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-2xl">
          ...
          <div className="flex items-center gap-3 bg-gradient-to-r from-purple-600 to-fuchsia-500 px-4 py-3 text-white">
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="truncate text-[11px] text-white/80">{p.subtitle}</p>
            </div>
            <button onClick={() => setOpen(false)} aria-label={p.close} className="text-white/80 transition-colors hover:text-white">
              <X className="h-5 w-5" />
            </button>
          </div>
          ...
          <div ref={scrollRef} className="flex-1 space-y-3 overflow-y-auto bg-slate-50 p-3">
            ...
            {loading && (
              <div className="flex justify-start">
                <div className="rounded-2xl bg-white px-3 py-2 text-sm text-slate-400 shadow-sm">{p.typing}</div>
              </div>
            )}
          </div>
